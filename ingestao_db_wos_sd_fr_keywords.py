...
def df_wos_sd_fullrecord_metadata_keywords(df):
    ...
    df.rename(columns={'ID': 'ID_WOS', 'KEYWORD': 'KEYWORDS'}, inplace=True)
...
if __name__ == '__main__':
    ...
    coluna = 'sd_fullrecord_metadata_keywords'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_sd_fullrecord_metadata_keywords(tabela)
    
    table_name = 'WOS_SD_FR_KEYWORDS'
