...
def df_wos_dd_citation_related_tc_list_cc(df):
    ...
    df['LOCAL_COUNT'] = df['LOCAL_COUNT'].astype(int)

    df['DEDUP_TOTAL_COUNT'] = df['DEDUP_TOTAL_COUNT'].astype(int)
...
if __name__ == '__main__':
    ...
    coluna = 'dd_citation_related_tc_list_cc'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_dd_citation_related_tc_list_cc(tabela)
    
    table_name = 'WOS_DD_CIT_REL_TC_LIST_CC'
    
    query_cria_tabela = f"""
                            CREATE TABLE {table_name} (
                                                        ID_WOS NUMBER,
                                                        FUNC_CLASS VARCHAR2(50),
                                                        LOCAL_COUNT NUMBER,
