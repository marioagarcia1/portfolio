...
def df_wos_sd_fullrecord_metadata_normalized_doctypes(df):
    ...
    df.rename(columns={'ID': 'ID_WOS', 'DOCTYPE':'DOCS_TYPE'}, inplace=True)
...
if __name__ == '__main__':
    ...
    coluna = 'sd_fullrecord_metadata_normalized_doctypes'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_sd_fullrecord_metadata_normalized_doctypes(tabela)
    
    table_name = 'WOS_SD_FR_NORM_DOCS'
    
    query_cria_tabela = f"""
                            CREATE TABLE {table_name} (
                                                        "ID_WOS" NUMBER, 
                                                        "DOCS_TYPE" VARCHAR2(50),
