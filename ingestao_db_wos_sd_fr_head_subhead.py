...
def df_wos_sd_fullrecord_metadata_cat_head_subhead(df):
    ...
    select_columns =   ['ID','HEADING', 'SUBHEADING']
...
if __name__ == '__main__':
    ...
    coluna = 'sd_fullrecord_metadata_cat_head_subhead'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_sd_fullrecord_metadata_cat_head_subhead(tabela)
    
    table_name = 'WOS_SD_FR_HEAD_SUBHEAD'
    
    query_cria_tabela = f"""
                            CREATE TABLE {table_name} (
                                                        ID_WOS NUMBER,
                                                        HEADING VARCHAR2(100),
                                                        SUBHEADING VARCHAR2(100),
