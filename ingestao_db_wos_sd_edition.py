...
def df_wos_sd_edition(df):
    ...
    select_columns =   ['ID','WUID','EDITION']
...
if __name__ == '__main__':
    ...
    coluna = 'sd_summary_EWUID'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_sd_edition(tabela)
    
    table_name = 'WOS_SD_EDITION'
    
    query_cria_tabela = f"""
                            CREATE TABLE {table_name} (
                                                        ID_WOS NUMBER,
                                                        WUID VARCHAR2(250),
                                                        EDITION VARCHAR2(250),
