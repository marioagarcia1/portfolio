...
for i in log_exec:
    ...
    if 'ERRO' in i[0].upper() or 'PROBLEMA' in i[0].upper() or i[1] != '':
        ...
        result = {'data processamento': data_processamento.strftime('%Y-%m-%d') ,
                  'arquivo': i[2].split(':')[1],
                  'status': 'ERRO',
                  'log': f"{i[0]}   {i[1]}"}
    else:
...
df.to_csv(f"logs/log_stage_execucao_wos_{data_processamento.strftime('%Y%m%d')}.csv", sep = ';', index = False)
