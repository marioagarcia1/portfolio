...
def df_wos_sd_sum_doctypes(df):
    
    df = df.groupby(['id']).agg({'doctype': '|'.join})
...
if __name__ == '__main__':
    ...
    coluna = 'sd_summary_doctypes'

    caminho = f'{pasta_dados}/{coluna[3:]}/arquivo_wos_{coluna}.parquet'

    tabela = pd.read_parquet(caminho)
    
    tabela = df_wos_sd_sum_doctypes(tabela)
    
    table_name = 'WOS_SD_SUM_DOCTYPES'
